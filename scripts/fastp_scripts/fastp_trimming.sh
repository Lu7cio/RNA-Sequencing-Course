...
# Get list of R1,R2 raw data files and select one for this array task
...
#-I : input R2 fastq file
#-o : output trimmed R1 fastq file
#-O : output trimmed R2 fastq file
#--html : output html report file
#--json : output json report file
#--detect_adapter_for_pe : auto-detect adapters for paired-end data
#--thread 1 : use 1 thread
#--bind : bind mount directories from host to container
#"$R1" : input R1 fastq file to process
#"$R2" : input R2 fastq file to process
#"$SIF_PATH" : path to the Apptainer container with fastp installed
